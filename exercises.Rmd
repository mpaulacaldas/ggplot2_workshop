...
knitr::opts_chunk$set(
  echo = TRUE, 
  error = TRUE, # because of hrbrthemes
  fig.path = "figures/"
  )
```
...
**TODO**: He says that you can also use patchwork with base R plots, but the code chunk below doesn't return anything.
...
The `&` allows us to modify all plots in our collection, the `+` operator will only work on the last layer.
...
- You can use `patchwork::wrap_plots()` if you have many plots that you are producing programmatically and/or you don't know how many plots you will have in total.
...
If you have a colour mapping, then you are not required to do this because the group aesthetic will be derived from this aesthetic.
